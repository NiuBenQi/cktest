...
public interface ApiRequestParamMapper extends BaseMapper<ApiRequestParam> {
    ...
    @Select("SELECT DISTINCT t2.* ,t1.api_request_param_value value FROM case_param_value t1 LEFT JOIN api_request_param t2 ON t1.api_request_param_id = t2.id WHERE t1.case_id = #{caseId}")
